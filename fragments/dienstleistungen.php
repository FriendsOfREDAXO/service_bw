<div class="service-bw service-bw-dienstleistungen"><?php
...
echo '<h2>Dienstleistungen - Service-BW</h2>';
...
foreach ($list as $key => $value) {
    ...
    $value = array_map(static function ($item) {
        $item['url'] = rex_getUrl(rex_article::getCurrentId(), '', ['dienstleistung' => $item['id']]);
        return $item;
    }, $value);
    echo '<ul>';
